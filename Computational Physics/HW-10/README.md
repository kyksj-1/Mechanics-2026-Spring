...
我选择以 John J. Hopfield 的研究视角来组织这份报告。原因是本题的耦合矩阵

$$
J_{ij}=\frac{1}{N}\sum_{\mu=1}^p v_i^\mu v_j^\mu,\quad J_{ii}=0
$$

正是 Hopfield 网络中 Hebb 规则的形式；题目的核心物理不是单纯找某个自旋模型的低能态，而是理解一组随机自旋构型如何成为能量景观中的吸引子。因此，用 Hopfield 的“能量函数 + 联想记忆 + 容量限制”的语言最自然。
...
结果表明，对 $N=2000,p=10$，所有带噪声的初态都回到了对应的记忆模式。末态与对应 $\mathbf{v}^\mu$ 的相似度全部为 1，而不同模式之间的最大绝对 overlap 只有 `0.048000`，说明这些随机模式近似正交，串扰很弱。
